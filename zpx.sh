...
if [ "$CLEAN_BUILD" == 'YES' ]
        ...
        echo;
         make clean
         make mrproper
        make ARCH=$ARCH CROSS_COMPILE="$(command -v ccache) $KERNEL_TOOLCHAIN"  $KERNEL_DEFCONFIG
fi
...
cp -r ./arch/arm/boot/dt.img $ANYKERNEL/anykernel/dtb
...
rm -rf $ANYKERNEL/anykernel/zImage;
rm -rf $ANYKERNEL/anykernel/dtb;
